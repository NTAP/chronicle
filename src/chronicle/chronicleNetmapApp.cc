...
{
	//TODO: man page
	std::cerr << "./chronicle_netmap -iethX -iethY ...\n"
...
{
	std::list<char *> netmapNics;
	std::list<char *>::const_iterator it;
	uint32_t batchSize = NETMAP_MAX_BATCH_SIZE;	//ring size
	...
	bool toCopy = true, enableAnalytics = false;
	char option;
	...
	// error handling
	if (netmapNics.size() == 0) {
		std::cerr 
			<< "ERROR: At least one network interface needs to be specified!\n";
		usage();
		exit(EXIT_FAILURE);
	}
	...
	Chronicle *chronicle = new Chronicle(netmapNics.size(), fileno(stdin), 
		pipelineType, pipelineNum, outputFormat, numOutputModules, snapLen,
		enableAnalytics, &supervisorCb, &statCb);

	// adding netmap NICs
	for (it = netmapNics.begin(); it != netmapNics.end(); it++) {
		Interface *i = new NetmapInterface(*it, batchSize, toCopy, snapLen);		
