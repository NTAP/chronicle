#!/usr/bin/python
"""
A wrapper for anonymizing and encrypting Chronicle traces
"""
...
def anonymizeFile(inputDir, inputFile):
	...
	command = './anonymizeNfs3Trace -c lzf --infile=' + os.path.join(inputDir, inputFile) + ' --outfile=' + os.path.join(outputDir, inputFile + '.anon') + ' -k ' + key 
	print command
	runCommand(command)
	if passphrase != None:
		...
		if validateAES_GPG(outputDir, inputFile + '.anon.gpg'):
			...
			print 'FAILED encryption for %s' % os.path.join(outputDir, inputFile + '.anon.gpg')
			logging.debug('ERROR: failed encryption for {}'.format(os.	path.join(outputDir, inputFile + '.anon.gpg')))
	if remove:
...
class File_Anonymizer(threading.Thread):
...
if os.path.exists(inputDir) and os.path.exists(outputDir) and os.path.exists(key):
	...
	exit(1)
logging.basicConfig(filename=os.path.join(outputDir, 'anonymizer.log'),level=logging.DEBUG)
